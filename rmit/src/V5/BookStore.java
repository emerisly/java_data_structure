...
public class BookStore {
	...
	public static int viewCart() {
		if (count<=1) {
			...
			for(int i=1;i<count;i++) {
				System.out.println(i+". " + bookList[bookInCart[i]].title + " -- " + bookList[bookInCart[i]].author);
			}
		}
		return 0;
	...
	public static int removeBook() {
		...
		if (remove == 0) {
			...
			for(int i=1;i<count;i++) {
				System.out.println("Selected: " + bookList[bookInCart[i]].title);
			}	
				System.out.println("Do you want to remove it: ");
				String cancel = myScanner.next();
				if (cancel.toLowerCase().equals("yes")) {
					for(int i=1;i<count;i++) {
						bookInCart[i+1]=bookInCart[i];
						bookInCart[count] = 0;
						count --;
						if (isPhysical[i]==2) {
							totalPrice -=50;
						}
						else if (isPhysical[i]==1) {
							totalPrice -=8;
						}
					}
					System.out.println("Item removes form Shopping Cart");
				}
				else if (cancel.toLowerCase().equals("no")) {
					return 0;
				}
				else {
					System.out.println("Sorry, that is an invalid option!");
